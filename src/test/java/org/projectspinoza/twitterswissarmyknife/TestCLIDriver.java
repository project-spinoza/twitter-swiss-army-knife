...
public class TestCLIDriver {
	...
	public void testDumpHomeTimeLine() throws Exception {
		...
		ResponseList<Status> result = (ResponseList<Status>) cliDriver
				.getHomeTimeLine(twitter, subCommand);
	...
	public void testDumpAccountSettings() throws Exception {
		...
		AccountSettings result = (AccountSettings) cliDriver
				.getAccountSettings(twitter, subCommand);
	...
	public void testDumpOwnRetweets() throws Exception {
		...
		ResponseList<Status> result = (ResponseList<Status>) cliDriver
				.getOwnRetweets(twitter, subCommand);
	...
	public void testDumpMentionsTimeLine() throws Exception {
		...
		ResponseList<Status> result = (ResponseList<Status>) cliDriver
				.getMentionsTimeLine(twitter, subCommand);
	...
	public void testDumpBlockList() throws Exception {
		...
		PagableResponseList<User> result = (PagableResponseList<User>) cliDriver
				.getBlockList(twitter, subCommand);
	...
	public void testDumpUserTimeLine() throws Exception {
		...
		Mockito.when(rateLimitStatus.getRemaining()).thenReturn(0);
		List<Status> result = (List<Status>) cliDriver.getUserTimeLine(twitter,
				subCommand);
	...
	public void searchUsers() throws Exception {
		...
		Mockito.when(twitter.searchUsers(keywords, 1)).thenReturn(
				userPagableResponseList);
		Mockito.when(userPagableResponseList.getRateLimitStatus()).thenReturn(
				rateLimitStatus);
		Mockito.when(rateLimitStatus.getRemaining()).thenReturn(0);
		List<ResponseList<User>> result = (List<ResponseList<User>>) cliDriver
				.searchUsers(twitter, subCommand);
	...
	public void testDumpFriendList() throws TwitterException, IOException {
		...
		Mockito.when(twitter.getFriendsList(uId, -1)).thenReturn(
				userPagableResponseList);
		Mockito.when(userPagableResponseList.getRateLimitStatus()).thenReturn(
				rateLimitStatus);
		Mockito.when(rateLimitStatus.getRemaining()).thenReturn(0);
	...
	public void testDumpListStatuses() throws Exception {
		...
		PowerMockito.whenNew(Paging.class).withArguments(1, 50)
				.thenReturn(page);
		Mockito.when(twitter.getUserListStatuses(lId, page)).thenReturn(
				statuses);
		Mockito.when(page.getPage()).thenReturn(181);

		List<Status> result = (List<Status>) cliDriver.getListStatuses(twitter,
				subCommand);
	...
	public void testDumpFollowersList() throws TwitterException, IOException {
		...
		Mockito.when(twitter.getFollowersList(uId, -1)).thenReturn(
				userPagableResponseList);
		Mockito.when(userPagableResponseList.getRateLimitStatus()).thenReturn(
				rateLimitStatus);
		Mockito.when(rateLimitStatus.getRemaining()).thenReturn(0);
		List<PagableResponseList<User>> result = (List<PagableResponseList<User>>) cliDriver
				.getFollowersList(twitter, subCommand);
	...
	public void testDumpFavourites() throws Exception {
		...
		ResponseList<Status> result = (ResponseList<Status>) cliDriver
				.getFavourites(twitter, subCommand);
	...
	public void testDumpSavedSearches() throws Exception {
		...
		ResponseList<SavedSearch> result = (ResponseList<SavedSearch>) cliDriver
				.getSavedSearches(twitter, subCommand);
	...
	public void testDumpUserSuggestions() throws Exception {
		...
		ResponseList<User> result = (ResponseList<User>) cliDriver
				.getUserSuggestions(twitter, subCommand);
	...
	public void testDumpMemberSuggestions() throws Exception {
		...
		ResponseList<User> result = (ResponseList<User>) cliDriver
				.getMemberSuggestions(twitter, subCommand);
	...
	public void testDumpSugeestedUserCats() throws Exception {
		...
		ResponseList<Category> result = (ResponseList<Category>) cliDriver
				.getSugeestedUserCats(twitter, subCommand);
	...
	public void testDumpTweets() throws Exception {
		...
		Mockito.when(queryResult.getRateLimitStatus()).thenReturn(
				rateLimitStatus);
		Mockito.when(rateLimitStatus.getRemaining()).thenReturn(0);

		List<List<Status>> result = (List<List<Status>>) cliDriver.getTweets(
				twitter, subCommand);
